...
jQuery.fn.tableBarChart = function(targetDiv, caption, reverseGroup) {
	...
	function layout() {
		...
		target.find('.y-axis li').css('height', 70).css('width', '100%');
		var barGroupHeight = (target.find('.y-axis li').height() + 1) * 4;
	...
	function render() {
		...
		graphContainer.append(getAxisHTML(getYAxisArray(4), 'y-axis'));
